...
const totalChequeIn = ref(0);
const totalChequeOut = ref(0);
const loading = ref(true);

const dailyChequeData = (ChequeIn, ChequeOut) => {
    dailyChequeInTransactions.value = ChequeIn;
    dailyChequeOutTransactions.value = ChequeOut;
    totalChequeIn.value = ChequeIn.reduce((acc, curr) => acc + curr.treasuryVoucher.totalAmount, 0);
    totalChequeOut.value = ChequeOut.reduce((acc, curr) => acc + curr.treasuryVoucher.totalAmount, 0);
    loading.value = false;
}

defineExpose({ loading, totalChequeIn, totalChequeOut, dailyChequeData });
</script>
<template>
    ...
    <DataTable :value="dailyChequeInTransactions" class="transaction-table" size="small" :loading="loading">
        <template #empty>
    ...
    <div class="grid w-full justify-center font-bold relative top-2.5">
        <span>Total Ingresos: {{ currencyNumber(totalChequeIn) }}</span>
    </div>
    ...
    <DataTable :value="dailyChequeOutTransactions" class="transaction-table" size="small" :loading="loading">
        <template #empty>
            <template v-if="!loading">
                <div class="flex justify-center items-center text-red-500">
                    Sin movimientos
    ...
    <div class="grid w-full justify-center font-bold relative top-2.5">
        <span>Total Egresos: {{ currencyNumber(totalChequeOut) }}</span>
    </div>
</template>